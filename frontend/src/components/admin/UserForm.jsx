...
//  * Props:
//  * - mode: "create" | "edit"
//  * - initialValues: { id, name, email, role_id, is_active, avatar_url, created_at, updated_at }
//  * - onSubmit: optional callback(data) sau khi lưu thành công
//  */
// export default function UserForm({ mode = "create", initialValues = {}, onSubmit: onSubmitProp }) {
...
//         typeof initialValues.is_active === "boolean" ? initialValues.is_active : true
//     );
...
//     // ===== Sync when initialValues changes (important for edit page) =====
//     useEffect(() => {
...
//         (async () => {
//             try {
//                 const url = summaryApi.url(`${summaryApi.role.list}${toQuery({ withCounts: false })}`);
...
//     //         const url = summaryApi.url(summaryApi.upload.user.single);
...
//             // 3) Gọi đúng endpoint /upload/users
//             const url = summaryApi.url(summaryApi.upload.user.single);
//             const res = await fetch(url, {
//                 method: "POST",
//                 headers: { ...authHeaders() }, // KHÔNG set Content-Type khi dùng FormData
...
//             // 4) Đọc kết quả & map trường url linh hoạt
//             const json = await res.json().catch(() => ({}));
//             if (!res.ok || json?.success === false) {
//                 const msg = json?.message || `Upload thất bại (${res.status})`;
//                 toast.error(msg);
//                 return;
//             }

//             const uploadedUrl =
//                 json?.url ?? json?.data?.url ?? json?.file?.url ?? "";
...
//     //     e.stopPropagation();
//     //     setDragOver(false);
//     //     handleUploadAvatar(e.dataTransfer.files?.[0]);
//     // };
//     // (giữ nguyên phần drag & drop)
//     const onDrop = (e) => {
//         e.preventDefault();
//         e.stopPropagation();
//         setDragOver(false);
...
//         if (!email?.trim() || !isEmail(email)) return toast.error("Email không hợp lệ");
...
//                 name: name.trim(),
//                 email: email.trim(),
//                 role_id: roleId, // để nguyên string, BE tự parse/uuid/number tùy schema
//                 is_active: !!isActive,
...
//                 const url = summaryApi.url(summaryApi.user.create);
//                 const res = await fetch(url, {
//                     method: "POST",
...
//                 const url = summaryApi.url(summaryApi.user.update(id));
//                 const res = await fetch(url, {
//                     method: "PUT",
//                     headers: { "Content-Type": "application/json", ...authHeaders() },
...
//                                 onDrop={onDrop}
//                                 className={`rounded-2xl boarder-2 border-dashed ${dragOver ? "border-indigo-500 bg-indigo-50" : "border-slate-300 bg-slate-50"
...
//                                             />
//                                             <p className="text-xs text-slate-500 mt-1">
//                                                 Tối thiểu 8 ký tự, nên gồm chữ hoa, chữ thường và số.
//                                             </p>
//                                         </div>
...
//                                                 className="w-full rounded-xl border border-slate-300 px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
//                                             >
...
//                                 ) : (
//                                     <>
//                                         {/* Edit mode: only Role, keep 2-col layout */}
//                                         <div>
//                                             <label className="block text-sm font-medium mb-1">Vai trò</label>
//                                             <select
//                                                 value={roleId}
...
//                                             </select>
//                                         </div>
//                                         <div /> {/* spacer để giữ layout 2 cột */}
//                                     </>
//                                 )}
//                             </div>
//                         </section>
