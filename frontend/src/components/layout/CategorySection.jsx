...
/* --- 3. CUSTOM HOOK: useItemsPerRow --- */
function useItemsPerRow() {
  const getItemsCount = () => {
    if (typeof window === "undefined") return 10; // fallback khi SSR
...
const CategoryItem = memo(({ data }) => {
  ...
  // ✅ Placeholder mới
  const PLACEHOLDER = "https://via.placeholder.com/220x300?text=Category";

  const rawImageUrl = data?.image_url;
  const imageSrc = isValidImageUrl(rawImageUrl) ? rawImageUrl : PLACEHOLDER;

  const handleImageError = (e) => {
    const target = e.currentTarget;
    // Nếu đã dùng placeholder rồi mà vẫn lỗi thì thôi, không set nữa để tránh loop
    if (target.src.includes("via.placeholder.com/220x300?text=Category")) {
      target.onerror = null;
      return;
    }
    target.onerror = null;
...
/* --- 5. MAIN COMPONENT: CategorySection --- */
export default function CategorySection() {
  const { categories, loading } = useCategories();
  const itemsPerRow = useItemsPerRow();

  // Chia categories thành các slide
  ...
  // Đảm bảo index luôn hợp lệ khi số slide thay đổi
  useEffect(() => {
    setIndex((prev) => {
      if (prev >= totalSlides) {
