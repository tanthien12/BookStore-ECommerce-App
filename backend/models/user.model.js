...
function buildAdminFilter({
  ...
  if (
    ...
    if (typeof is_active === "boolean") {
      activeBool = is_active;
    } else {
      const v = String(is_active).trim().toLowerCase();
      // Hỗ trợ "1"/"0", "true"/"false", "yes"/"no"
      activeBool =
...
async function adminFindMany({
  ...
  const { where, params } = buildAdminFilter({
    q,
    role_id,
    is_active,
    created_from,
...
async function adminUpdateUser(
  id,
  { name, email, role_id, is_active, avatar_url }
) {
  const sets = [];
  const params = [];
  let i = 1;
