...
import { products } from "../data/products"; // dữ liệu tạm thời mock
...
const ShippingCard = () => (
  <div className="rounded-xl border border-gray-200 bg-white p-4 space-y-3">
    ...
    <div className="text-sm text-gray-600">
      Dự kiến nhận: 1-3 ngày · Miễn phí từ 299k
    </div>
...
//
// ========== Component chính ProductDetail sau khi tích hợp review ==========
//
export default function ProductDetail() {
  const { id } = useParams(); // đây cũng chính là bookId để gọi API review
  const navigate = useNavigate();

  // sản phẩm mock (sau này bạn sẽ thay = fetch /api/books/:id)
  ...
  // ================== REDUX: Lấy user đang đăng nhập ==================
  const currentUser = useSelector((state) => state.user.user);

  // ================== REVIEW STATE ==================
  const [reviews, setReviews] = useState([]);

  // Hàm load danh sách review từ backend
  const loadReviews = () => {
    reviewApi
      .fetchAll(id) // id ở url cũng là bookId
      .then((list) => setReviews(list))
      .catch(() => {});
  };

  // Lần đầu / khi đổi sản phẩm thì load review
  useEffect(() => {
    loadReviews();
  }, [id]);

  // Nếu không tìm thấy sản phẩm mock → điều hướng hoặc báo lỗi
  useEffect(() => {
    if (!product) {
    ...
    return (
      <div className="mx-auto max-w-7xl px-3 md:px-4 py-10">
        <div className="text-center">
          ...
          <Link to="/" className="mt-3 inline-block text-red-600 hover:underline">
            Về trang chủ
          </Link>
        </div>
      </div>
  ...
  // Lấy thông tin rating tổng quan từ product (nếu backend trả về)
  // Nếu mock chưa có thì fallback 0
  ...
  return (
    <div className="mx-auto max-w-7xl px-3 md:px-4 py-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        {/* ========== CỘT PHẢI: THÔNG TIN MUA HÀNG + MÔ TẢ + REVIEW ========== */}
        <div className="lg:col-span-3 space-y-4">
          {/* Thông tin chính: tên sách, giá, ship, số lượng, nút mua */}
          ...
          {/* ========== ĐÁNH GIÁ & NHẬN XÉT (đã tích hợp ReviewForm + ReviewCard) ========== */}
          <div className="rounded-2xl border border-gray-200 bg-white p-4">
            {/* Header + tổng quan rating */}
            <div className="mb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              ...
              {/* chỗ này sau bạn có thể thêm dropdown sắp xếp review */}
            </div>

            {/* Form người dùng gửi đánh giá */}
            <div className="mb-6">
              <ReviewForm
                bookId={id}
