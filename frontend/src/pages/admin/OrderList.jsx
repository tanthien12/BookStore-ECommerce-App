...
export default function OrderList() {
    ...
    // Đóng filter khi click ngoài / ESC
    useEffect(() => {
    ...
    // Đóng column menu khi click ngoài / ESC
    useEffect(() => {
    ...
    // Reset về page 1 khi đổi query hoặc filters
    useEffect(() => {
        setPage(1);
    ...
    // Fetch list orders từ backend (lọc theo status, from, to ở DB)
    useEffect(() => {
        let ignore = false;
    ...
    // Từ giờ FE không lọc local nữa, dùng trực tiếp data từ API
    ...
    useEffect(() => {
        const idsOnPage = visibleOrders.map((o) => o.id);
    ...
    const handleBulkDelete = async () => {
        ...
        try {
            ...
            if (successIds.length) {
                toast.success(
                    `Đã xóa ${successIds.length}/${ids.length} đơn hàng`
                );
                setOrders((prev) =>
                    prev.filter((o) => !successIds.includes(o.id))
                );
    ...
    // Export CSV: gọi API với full filter (q, status, from, to) và limit lớn
    const handleExportCSV = async () => {
        try {
            ...
            url.searchParams.set("limit", "5000"); // export tối đa 5000 đơn
            ...
            toast.success(`Đã export ${data.length} đơn hàng`);
        } catch (err) {
            console.error(err);
            toast.error(err.message || "Không export được CSV");
        }
    };
    ...
    const activeFilterCount = useMemo(() => {
        return [filters.status, filters.dateFrom, filters.dateTo].filter(Boolean)
            .length;
    }, [filters]);
    ...
    // Update order status (PATCH). Optimistic update + rollback on error.
    const handleUpdateStatus = async (orderId, newStatus) => {
        const order = orders.find((o) => o.id === orderId);
        const oldStatus = order?.status;

        // Không có order hoặc không thay đổi -> bỏ qua
        if (
            !order ||
        ...
        try {
            const url = summaryApi.url(summaryApi.order.updateStatus(orderId));
            ...
            if (!res.ok || json?.success === false) {
                const msg =
                    json?.message ||
                    `Cập nhật trạng thái thất bại: ${res.status}`;
                throw new Error(msg);
            }
            ...
            <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {loading ? (
                                ...
                                visibleOrders.map((order) => {
                                    ...
                                    const updating =
                                        updatingStatusIds.has(order.id);
                                    return (
                                        <tr
                                            key={order.id}
                ...
                <div className="flex flex-col gap-3 border-t border-gray-200 px-4 py-3 text-sm text-gray-600 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="rounded-lg bg-blue-600 px-3 py-1.5 text-white">
                            {page}
                        </span>
                        <button
                            className="rounded-lg border border-gray-300 px-3 py-1.5 hover:bg-gray-50 disabled:opacity-50"
...
//         1,
//         Math.ceil(Math.max(total, 0) / pageSize)
//     );
...
//         total === 0
//             ? 0
//             : Math.min(total, (page - 1) * pageSize + filteredOrders.length);
...
//                             hasLocalDateFilter ? (
//                                 <>
//                                     Đang hiển thị {filteredOrders.length} đơn
//                                     trong trang hiện tại / {total}
//                                 </>
//                             ) : (
//                                 <>
...
//                                 * Lọc theo ngày áp dụng trên danh sách đang
//                                 hiển thị.
//                             </div>
//                         )}
//                     </div>
