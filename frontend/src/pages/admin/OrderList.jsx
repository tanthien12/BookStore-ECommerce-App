...
export default function OrderList() {
    ...
    // ---------- NEW: state to track statuses being updated ----------
    const [updatingStatusIds, setUpdatingStatusIds] = useState(() => new Set());

    // Utility to mark id updating
    ...
    const handleUpdateStatus = async (orderId, newStatus) => {
        ...
        if (!order || (String(oldStatus || "").toLowerCase() === String(newStatus || "").toLowerCase())) return;
        ...
        try {
            const url = summaryApi.url(summaryApi.order.update(orderId));
            const res = await fetch(url, {
                method: "PUT",
            ...
            if (!res.ok) {
                ...
                let msg = `Cập nhật trạng thái thất bại: ${res.status}`;
                try {
            ...
            // success: ensure server returned updated item (optional)
            toast.success("Cập nhật trạng thái thành công");
            // optionally update the single order with server response if provided
            if (json && json.item) {
                setOrders((prev) => prev.map((o) => (o.id === orderId ? { ...o, ...json.item } : o)));
            }
        } catch (err) {
            console.error(err);
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {loading ? (
                                ...
                                filteredOrders.map((order) => {
                                    ...
                                    const updating = updatingStatusIds.has(order.id);
                                    return (
                                            ...
                                            {showCols.status && (
                                                <td className="p-3">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {/* small badge to show visual color */}
                                                        <span
