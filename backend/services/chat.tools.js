...
function isPgTrgmMissingError(e) {
  const msg = e?.message || "";
  return /similarity\(/i.test(msg) || /pg_trgm/i.test(msg);
}
...
async function tool_search_books(args) {
  ...
  try {
    ...
    if (!isPgTrgmMissingError(e)) throw e;

    // Fallback: bỏ similarity, chỉ còn ILIKE + filter
    const whereFallback = where.map((cond, idx) =>
      idx === 0 ? cond.replace(`( ${simExpr} ) > 0.1 OR `, "") : cond
    );
    const sqlFallback = `
        ${baseSelect}
        WHERE ${whereFallback.join(" AND ")}
        ORDER BY b.rating_avg DESC NULLS LAST, b.created_at DESC
        LIMIT 5
      `;
    const res2 = await pool.query(sqlFallback, params);
    rows = res2.rows;
  }
...
async function tool_get_similar_books(args) {
  ...
  if (book_id) {
    ...
    try {
      ...
      if (!isPgTrgmMissingError(e)) throw e;
      const whereFallback = `(b.title ILIKE '%'||$1||'%' OR b.author ILIKE '%'||$1||'%')`;
      const sqlFallback = `
        ${baseSelect}
        WHERE ${whereFallback}
        ORDER BY b.rating_avg DESC NULLS LAST, b.created_at DESC
...
// ====== Tool registry ======
// Lưu ý: KHÔNG còn đăng ký add_to_cart cho chatbot nữa.
const TOOL_REGISTRY = {
  search_books: {
    name: "search_books",
  ...
  get_similar_books: {
    ...
    schema: getSimilarBooksSchema,
    exec: (userId, args) => tool_get_similar_books(args),
  },
  filter_by_price: {
    name: "filter_by_price",
    schema: filterByPriceSchema,
    exec: (userId, args) => tool_filter_by_price(args),
  },
  list_best_sellers: {
    name: "list_best_sellers",
    schema: listBestSellersSchema,
    exec: (userId, args) => tool_list_best_sellers(args),
  },
};
