...
const COL_KEY = "admin.users.table.columns.v2";
...
export default function UserList() {
    ...
    // Fetch Users (server-side filter & paging)
    useEffect(() => {
        let ignore = false;
        ...
        (async () => {
            try {
                ...
                if (filters.roleId)
                    url.searchParams.set("role_id", filters.roleId);
                if (filters.dateFrom)
            ...
            for (const r of rows) {
                lines.push(
                    [
                        ...
                        esc(r.name),
                        esc(r.email),
                        esc(r.role_name || r.role || ""),
                        esc(r.is_active ? "active" : "inactive"),
                        esc(r.created_at || ""),
                    ].join(",")
                );
            }
                                    ...
                                    <div>
                                        ...
                                        >
                                            ...
                                            {rolesLoading ? (
                                                ...
                                                roles.map((role) => {
                                                    const count =
                                                        role.users_count ?? role.usersCount ?? role.count ?? null;

                                                    // ưu tiên role_name giống backend, fallback sang name/code
                                                    const label =
                                                        role.role_name || role.name || role.code || role.slug || "Không rõ";
                        ...
                        <table className="min-w-full text-sm">
                            <thead className="bg-gray-100 text-gray-600">
                                <tr>
                                    ...
                                    <th className="w-28 p-3 text-right">
                                        Actions
                                    </th>
                                </tr>
                            </thead>
                                    ...
                                    visibleItems.map((u) => (
                                        ...
                                        >
                                            ...
                                            {showCols.status && (
                                                <td className="p-3 align-middle">
                                                    <span
                                                        className={`inline-flex items-center px-2 py-0.5 rounded-full border text-xs ${u.is_active
                                                            ? STATUS_BADGE.active
                                            ...
                                            <td className="p-3 align-middle">
                                                <div className="flex items-center justify-end gap-2">
                                                    ...
                                                    {/* Xóa (chưa implement backend) */}
                                                    <button
                                                        onClick={() =>
                                                            toast.info(
                                                                "Delete user sẽ implement sau"
                                                            )
                                                        }
