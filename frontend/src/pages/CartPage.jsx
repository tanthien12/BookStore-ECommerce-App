...
export default function CartPage() {
  ...
  // tạm tính chỉ theo SP được chọn
  const displaySubtotal = useMemo(() => {
    if (selectedProductIds.length > 0) {
      const chosenItems = cart.filter((item) =>
        selectedProductIds.includes(item.productId)
      );
      return chosenItems.reduce((acc, it) => acc + it.price * it.qty, 0);
    }
    return 0; 
  }, [cart, selectedProductIds]);

  //  chỉ cho thanh toán khi đã chọn ít nhất 1 sp
  const canCheckout = selectedProductIds.length > 0;
    ...
    return (
      <div className="mx-auto max-w-5xl px-3 md:px-4 py-6">
        <h1 className="text-xl font-bold mb-4">
          GIỎ HÀNG <span className="text-gray-500 text-sm">(0 sản phẩm)</span>
        </h1>
    ...
    <div className="mx-auto max-w-5xl px-3 md:px-4 py-6">
      ...
      <div className="rounded-xl border border-gray-300 bg-white overflow-hidden">
        ...
        <div className="divide-y divide-gray-200 bg-white">
          {cart.map((item) => {
            const isChecked = selectedProductIds.includes(item.productId);
              ...
              >
                ...
                <div className="w-full md:w-[220px] flex items-start md:items-center md:justify-between gap-4 text-sm">
                  ...
                  <QuantityInline
                    ...
                    onManual={(val) => updateQty(item.productId, Number(val))}
                  />
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 px-4 py-4 border-t border-gray-300 bg-gray-50 text-sm">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:gap-4 w-full md:w-auto text-right md:text-left">
            ...
            <button
              ...
              disabled={!canCheckout} // (THÊM)
              className={`inline-flex items-center justify-center rounded-lg px-5 py-2 text-white text-base font-semibold ${
                canCheckout
