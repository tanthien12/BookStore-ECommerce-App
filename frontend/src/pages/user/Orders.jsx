...
// Lấy danh sách key để dùng cho dropdown filter
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-5">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mb-4">
                ...
                <div className="flex items-center gap-2">
                    {/* 2. Sửa Dropdown Filter: Dùng STATUS_MAP để hiển thị option */}
                                ...
                                <tbody>
                                    {items.map(o => (
                                        <tr key={o.id} className="border-t">
                                            ...
                                            {/* 3. Sửa hiển thị trong bảng: Dùng Map thay vì hiển thị trực tiếp */}
                                            <td className="p-2 text-center">
                                                {STATUS_MAP[o.status] || o.status}
                                            </td>
                                            <td className="p-2 text-right">{money(o.grand_total)}</td>
                                            <td className="p-2">{o.placed_at ? new Date(o.placed_at).toLocaleString('vi-VN') : ''}</td>
            ...
            {detail && (
                <div className="fixed inset-0 bg-black/30 flex items-center justify-center p-4 z-50" onClick={() => setDetail(null)}>
                    <div className="bg-white rounded-2xl w-full max-w-2xl p-4" onClick={e => e.stopPropagation()}>
                        ...
                        {/* 4. Sửa hiển thị trong Modal */}
                        <div className="text-sm text-gray-700 mb-3">
                            Trạng thái: <b>{STATUS_MAP[detail.order.status] || detail.order.status}</b> · Tổng: <b>{money(detail.order.grand_total)}</b>
                        </div>
