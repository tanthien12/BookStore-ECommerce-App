...
export default function CategoryList() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                // ⬇️ gửi filter xuống backend
                if (filters.hasProducts === "with")
                    url.searchParams.set("has_products", "with");
                else if (filters.hasProducts === "without")
                    url.searchParams.set("has_products", "without");
    ...
    // Không còn lọc local nữa, mọi thứ do DB làm
    const visibleItems = useMemo(() => items, [items]);
    const hasLocalFilters = false;

    const columnCount =
        1 + Object.values(showCols).filter(Boolean).length + 1; // select + visible cols + actions
    ...
    useEffect(() => {
        ...
        if (headerCbRef.current) {
            headerCbRef.current.indeterminate =
                checkedCount > 0 && checkedCount < visibleItems.length;
            headerCbRef.current.checked =
                checkedCount === visibleItems.length && visibleItems.length > 0;
        }
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const res = await fetch(summaryApi.url(summaryApi.category.delete(id)), {
                method: "DELETE",
            });
            if (!res.ok) {
        ...
        try {
            ...
            for (const id of ids) {
                const res = await fetch(
                    summaryApi.url(summaryApi.category.delete(id)),
                    {
                        method: "DELETE",
                    }
                );
    ...
    // ===== export CSV (lọc ở DB, không filter local) =====
    const handleExportCsv = async () => {
        try {
            ...
            a.download = `categories_${new Date()
                .toISOString()
                .slice(0, 19)
                .replace(/[:T]/g, "-")}.csv`;
    ...
    return (
        <div className="space-y-5">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="relative">
                        ...
                        {openColMenu && (
                            <div
                                ref={colMenuRef}
                                className="absolute right-0 mt-2 w-64 rounded-xl border border-gray-200 bg-white shadow-lg p-3 z-10"
                            >
            ...
            <div className="bg-white rounded-xl shadow border border-gray-200 overflow-hidden">
                ...
                {/* Pagination footer */}
                <div className="flex flex-col gap-3 px-4 py-3 bg-white border-t border-gray-200 text-sm text-gray-600 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        {total > 0 ? (
...
//         filters.hasProducts || filters.dateFrom || filters.dateTo
//     );
//     const columnCount =
//         1 + Object.values(showCols).filter(Boolean).length + 1; // select + visible cols + actions
...
//                 const count =
//                     c.productsCount ?? c.products_count ?? c.products?.length ?? 0;
//                 if (filters.hasProducts === "with" && Number(count) <= 0) return false;
//                 if (filters.hasProducts === "without" && Number(count) > 0) return false;
...
//                                         0;
//                                     return (
//                                         <tr key={c.id} className="hover:bg-gray-50 border-b border-gray-400">
//                                             <td className="w-12 p-0">
//                                                 <div className="h-16 flex items-center justify-center">
//                                                     <input
//                                                         type="checkbox"
...
//                                             )}
//                                             {showCols.products && (
//                                                 <td className="p-3 align-middle font-medium">{count}</td>
//                                             )}
...
//                                 * Bộ lọc áp dụng trên dữ liệu của trang hiện tại.
//                             </div>
//                         )}
//                     </div>
//                     <div className="flex items-center gap-2">
